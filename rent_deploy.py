    ...
    if start_date > end_date:
        ...
        with tabs[3]:
            ...
            if st.checkbox('SARIMAX Forecast', key="sarimax_forecast"):
                ...
                # Display the ticker table with Date, Value, and Value_diff without the index column
                # st.subheader("Ticker Data: Date, Value, and Value_diff")
                # ticker_data = filtered_data[['Date', 'Value', 'Value_diff']].tail(10).reset_index(drop=True)  # Reset index
                # st.table(ticker_data)
                ...
                # Confidence intervals for the future forecast
                confidence_intervals_future = sarimax_forecast_future.conf_int()
                lower_bounds_future = confidence_intervals_future.iloc[:, 0]
                upper_bounds_future = confidence_intervals_future.iloc[:, 1]
                ...
                # Create the forecast dataframe for SARIMAX
                forecast_df_future = pd.DataFrame({
                    'Date': forecast_index_future,
                    'Forecast': forecast_values_future,
                    'Lower Bound': lower_bounds_future.values,
                    'Upper Bound': upper_bounds_future.values
                })
                ...
                # Add SARIMAX forecast for future values
                fig_sarimax.add_trace(
                    go.Scatter(
                        x=forecast_df_future['Date'],
                        y=forecast_df_future['Forecast'],
                        mode='lines',
                        name='Future Forecast'
                    )
                )

                # Add shaded confidence interval for the future forecast
                fig_sarimax.add_trace(
                    go.Scatter(
                        x=pd.concat([forecast_df_future['Date'], forecast_df_future['Date'][::-1]]),
                        y=pd.concat([forecast_df_future['Upper Bound'], forecast_df_future['Lower Bound'][::-1]]),
                ...
                # Forecasting future values with a chart and ticker
                st.subheader("Forecasted Rent Values")

                # Forecast future rent values
                forecast_months = st.sidebar.slider('Select Forecast Period (Months):', 1, 36, 12, key="sarimax_forecast_months")
                sarimax_forecast_future = sarimax_results.get_forecast(steps=forecast_months)
                forecast_values_future = sarimax_forecast_future.predicted_mean
                ...
                # Reconstruct Actual Rent Values (reverse differencing)
                last_actual_value = filtered_data['Value'].iloc[-1]
                actual_rent_values = [last_actual_value]

                # Reconstruct actual rent values from differenced forecasts
                for diff in forecast_values_future:
                    actual_rent_values.append(actual_rent_values[-1] + diff)

                # Remove the initial last_actual_value
                actual_rent_values = actual_rent_values[1:]

                # Create the forecast dataframe for the chart and ticker
                forecast_df = pd.DataFrame({
                    'Date': forecast_index_future,
                    'Forecasted Rent Value': actual_rent_values,
                    'Lower Bound': lower_bounds_future.values,
                    'Upper Bound': upper_bounds_future.values
                })
                ...
                # Add the forecasted rent values line
                fig_forecast.add_trace(
                    go.Scatter(
                        x=forecast_df['Date'],
                        y=forecast_df['Forecasted Rent Value'],
                        mode='lines',
                        name='Forecasted Rent Value'
                    )
                )
                ...
                fig_forecast.update_layout(
                    title="Forecasted Rent Values with Confidence Interval",
                    xaxis_title="Date",
                    yaxis_title="Rent Value",
                    legend_title="Legend"
                )
